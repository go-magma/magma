...
func init() {
	pipelinedExpectedVal = strings.ReplaceAll(strings.ReplaceAll(pipelinedExpectedVal, "\n", ""), " ", "")
	marshaledSrc = strings.ReplaceAll(strings.ReplaceAll(marshaledSrc, "\n", ""), " ", "")
}
...
func TestMconfigMarshal(t *testing.T) {
	...
	assert.NoError(t, err)
	assert.Equal(t, pipelinedExpectedVal, strings.ReplaceAll(strings.ReplaceAll(pdJson, "\n", ""), " ", ""))

	marshaledRes, err := protos.MarshalMconfigToString(cfg)
	assert.NoError(t, err)
	assert.Equal(t, marshaledSrc, strings.ReplaceAll(strings.ReplaceAll(marshaledRes, "\n", ""), " ", ""))
}
