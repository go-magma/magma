...
import (
	...
	subscriber_streamer "github.com/go-magma/magma/modules/lte/cloud/go/services/subscriberdb/streamer"
...
func TestLTEStreamProviderServicer_GetUpdates(t *testing.T) {
	...
	t.Run("subscriber streamer", func(t *testing.T) {
		...
		assert.NoError(t, err)
		assert.Equal(t, protos.TestMarshal(&protos.DataUpdateBatch{Updates: want}), protos.TestMarshal(got))
	})
}
