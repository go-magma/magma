...
import (
	...
	swx_protos "github.com/go-magma/magma/modules/feg/cloud/go/protos"
	...
	"github.com/go-magma/magma/modules/feg/gateway/services/eap/providers/sim/metrics"
...
func createChallengeRequest(
	...
	for vlen := 0; vlen < sim.GsmTripletsNumber; {
		...
		if len(ans.SipAuthVectors) == 0 {
			return sim.EapErrorResPacket(
				identifier, sim.NOTIFICATION_FAILURE, codes.Internal,
		...
		for _, v := range ans.GetSipAuthVectors() {
			...
			if len(ra) < sim.RandAutnLen {
				return sim.EapErrorResPacket(
					...
					len(ra), sim.RandAutnLen, ans.String())
			}
