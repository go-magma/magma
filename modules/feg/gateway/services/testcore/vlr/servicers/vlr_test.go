...
import (
	...
	"github.com/go-magma/magma/modules/feg/gateway/services/csfb/servicers"
...
func TestVLRServer_Reset(t *testing.T) {
	...
	_, err := client.Reset(context.Background(), &orcprotos.Void{})
	assert.NoError(t, err)
...
func TestVLRServer_ConfigServer(t *testing.T) {
	...
	client := protos.NewMockCoreConfiguratorClient(conn)
	_, err := client.ConfigServer(context.Background(), &config)
	assert.NoError(t, err)
