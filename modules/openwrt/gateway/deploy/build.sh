...
if [ -n "${build}" ]; then
  ...
  cd "${magma_path}/gateway/go" || exit 1
  ...
  GOOS=linux GOARCH=arm go build -tags link_local_service -o "${bin_dir}" -ldflags="${LD_FLAGS}" magma/modules/feg/gateway/services/aaa/aaa_server
fi
